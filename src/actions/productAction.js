...
export const newReview = (reviewData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(`http://192.168.1.69:8080/api/products/review`, reviewData, config);
...
export const getAllReviews = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`http://192.168.1.69:8080/api/products/reviews?id=${id}`);
...
export const deleteReviews = (reviewId, productId) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(
      `http://192.168.1.69:8080/api/products/reviews?id=${reviewId}&productId=${productId}`
    );
